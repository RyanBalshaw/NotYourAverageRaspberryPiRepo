...
if __name__ == "__main__":
    ...
    recent_activity_id = strava_vis.get_latest_activity_code(env_path, activity_type="Run")

    df_recent_activity_stream = strava_vis.get_activity_stream(env_path, recent_activity_id)
    ...
    # Set up Roboto font
    roboto_regular = fm.FontProperties(fname=os.path.join(os.getcwd(), "fonts", "roboto", "Roboto-Regular.ttf"))
    roboto_bold = fm.FontProperties(fname=os.path.join(os.getcwd(), "fonts", "roboto", "Roboto-Bold.ttf"))
    font_awesome = fm.FontProperties(fname=os.path.join(os.getcwd(), "icons", "font-awesome", "Font Awesome 6 Free-Regular-400.otf"))
    font_awesome_brands = fm.FontProperties(
        fname=os.path.join(os.getcwd(), "icons", "font-awesome", "Font Awesome 6 Brands-Regular-400.otf"))
    ...
    gs = fig.add_gridspec(2, 2, width_ratios=[4, 1], height_ratios=[2, 1], hspace=0.3, wspace=0.3)

    # Run path colored by gradient
    ax_path = fig.add_subplot(gs[0, :])
    scatter = ax_path.scatter(lat_lng[:, 1], lat_lng[:, 0], c=gradient, cmap='viridis', s=3, alpha=0.7)
    ...
    ax_path.set_title('Run df_recent_activity_stream', fontproperties=roboto_bold, fontsize=14)
    ...
    ax_path.text(0.02, 0.98, f"Latest Run: {formatted_recent_timestamp}\nDistance: {recent_distance:.2f} km\nGear: {recent_gear}\nCalories: {recent_calories}",
                 transform=ax_path.transAxes, fontproperties=roboto_regular, fontsize=10,
    ...
    counts, bins, _ = ax_hr.hist(hrt, density=True, bins=50, alpha=0.6, color=secondary_color, edgecolor=accent_color)
    kde = scistats.gaussian_kde(hrt)
    xx = np.linspace(np.min(hrt), np.max(hrt), 100)
    ...
    ax_stats.text(0.5, 0.8, "Yearly Statistics:", va='center', ha='center', fontproperties=roboto_bold, fontsize=12,
                  color=text_color)
    stats = [
    ...
    ax_stats.text(0.5, 0.5, '\n'.join(stats), va='center', ha='center', fontproperties=roboto_regular, fontsize=12,
                  color=text_color)
    ax_stats.text(.6, .4, "\uf428", fontproperties=font_awesome_brands)
    ax_stats.axis('off')
    ...
    # Add main title
    fig.suptitle('Strava Run Analysis', fontproperties=roboto_bold, fontsize=16, y=0.98, color=text_color)
